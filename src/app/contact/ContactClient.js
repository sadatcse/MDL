  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        await showSuccess(
          'Message Sent!',
          'Thank you for contacting Mohammadi Developers Ltd. Our team will review your message and get back to you shortly.'
        );
        setFormData({
          name: '',
          email: '',
        ...
        showError('Submission Failed', result.error || 'Something went wrong. Please try again.');
      }
    } catch (error) {
      console.error('Submit failed:', error);
      showError('Error', 'A connection error occurred. Please try again later.');
    } finally {
      setLoading(false);
    }
  };
